...
async def run_fast_forwarding_cycle(config):
    """Быстрый цикл пересылки постов с минимальным интервалом (30 секунд)"""
    if not config.get_bool('ENABLE_FORWARDING', False):
        return
    
    logging.info("⚡ Запуск быстрого цикла пересылки (проверка каждые 30 секунд)")
