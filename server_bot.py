...
# Блокировка для синхронизации доступа к last_forwarded.txt
_forward_lock = asyncio.Lock()

# Словарь для отслеживания обрабатываемых постов (защита от одновременной обработки)
_processing_posts = {}
...
# ============================================================================
# EVENT-BASED FORWARDING: Новые функции для мгновенной пересылки через Bot API
# ============================================================================

def read_last_forwarded_events(filename=LAST_FORWARDED_FILE):
    """Читает last_forwarded.txt и возвращает словарь {(channel, post_id): True}
    Новый формат для event-based пересылки (без привязки к аккаунту)"""
    last_forwarded = {}
    try:
        if os.path.exists(filename):
            with open(filename, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        parts = line.split(' ')
                        # Новый формат: channel post_id (для event-based)
                        if len(parts) >= 2:
                            # Пытаемся прочитать как новый формат
                            try:
                                channel = parts[0].strip()
                                post_id = int(parts[1].strip())
                                last_forwarded[(channel, post_id)] = True
                                continue
                            except (ValueError, IndexError):
                                pass
                        
                        # Старый формат: phone channel channel_type post_id (для обратной совместимости)
                        if len(parts) >= 4:
                            phone = parts[0].strip()
                            ...
                            # Старый формат без channel_type
                            phone = parts[0].strip()
                            channel = parts[1].strip()
                        ...
                        continue
            logging.debug(f"📖 Загружено {len(last_forwarded)} записей из {filename} (event-based формат)")
        else:
...
def write_last_forwarded_events(data, filename=LAST_FORWARDED_FILE):
    """Записывает словарь {(channel, post_id): True} в last_forwarded.txt
    Новый формат для event-based пересылки"""
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("# Файл для отслеживания последних пересылок (event-based)\n")
            f.write("# Формат: channel post_id\n")
            f.write("# Пример: https://t.me/crptA92 12345\n\n")
            for (channel, post_id), _ in data.items():
                f.write(f"{channel} {post_id}\n")
        logging.debug(f"✅ Данные записаны в {filename} (event-based формат)")
    except Exception as e:
        logging.error(f"❌ Ошибка при записи {filename}: {e}")

# ============================================================================
# СТАРЫЕ ФУНКЦИИ (для обратной совместимости с polling)
# ============================================================================
...
def read_last_forwarded(filename=LAST_FORWARDED_FILE):
    ...
    СТАРАЯ функция для polling-based пересылки (deprecated)"""
    last_forwarded = {}
    try:
        if os.path.exists(filename):
            with open(filename, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        parts = line.split(' ')
                        # Новый формат: channel post_id (пропускаем для старой функции)
                        if len(parts) == 2:
                            continue
                        
                        # Старый формат: phone channel channel_type post_id
                        if len(parts) >= 4:
                            phone = parts[0].strip()
                            channel = parts[1].strip()
                            channel_type = parts[2].strip()
...
def write_last_forwarded(data, filename=LAST_FORWARDED_FILE):
    ...
    СТАРАЯ функция для polling-based пересылки (deprecated)"""
    try:
        # Сначала читаем event-based записи чтобы их не потерять
        event_data = read_last_forwarded_events(filename)
        event_data.update({(k[1], v): True for k, v in data.items() if isinstance(k, tuple) and len(k) >= 3})
        
        # Записываем в новом формате (event-based)
        write_last_forwarded_events(event_data, filename)
        
        # Также сохраняем в старом формате для совместимости
        with open(filename + '.old', 'w', encoding='utf-8') as f:
            f.write("# СТАРЫЙ ФОРМАТ (deprecated)\n")
            for (phone, channel, channel_type), post_id in data.items():
                f.write(f"{phone} {channel} {channel_type} {post_id}\n")
        logging.debug(f"✅ Данные записаны в {filename} (старый формат для совместимости)")
    except Exception as e:
        logging.error(f"❌ Ошибка при записи {filename}: {e}")
...
# ============================================================================
# EVENT-BASED FORWARDING: Мгновенная пересылка через Bot API при событии
# ============================================================================

async def forward_post_via_bot_api(msg, from_channel_link, to_channels, bot_token, config):
    """
    Пересылает пост через Telegram Bot API мгновенно при событии
    
    Args:
        msg: Сообщение из события Telethon
        from_channel_link: Ссылка на канал-источник
        to_channels: Список кортежей (channel_type, channel_link) для каналов назначения
        bot_token: Токен Telegram бота
        config: Конфигурация
    """
    try:
        # Получаем chat_id канала-источника
        source_chat_id = None
        try:
            if hasattr(msg, 'peer_id'):
                if hasattr(msg.peer_id, 'channel_id'):
                    # Для каналов формат: -100 + channel_id
                    source_chat_id = f"-100{msg.peer_id.channel_id}"
                elif hasattr(msg.peer_id, 'chat_id'):
                    source_chat_id = f"-{msg.peer_id.chat_id}"
                elif hasattr(msg.peer_id, 'user_id'):
                    source_chat_id = msg.peer_id.user_id
            ...
            if not source_chat_id:
                try:
                    ...
                    if hasattr(chat, 'id'):
                        chat_id = chat.id
                        # Для каналов добавляем префикс -100
                        if hasattr(chat, 'broadcast') and chat.broadcast:
                            source_chat_id = f"-100{abs(chat_id)}"
                        else:
                            source_chat_id = str(chat_id)
                except:
                    pass
        except Exception as e:
            logging.warning(f"⚠️ [BOT] Ошибка получения chat_id: {e}")
        
        if not source_chat_id:
            logging.error(f"❌ [BOT] Не удалось получить chat_id для канала {from_channel_link}")
            return False
        
        message_id = msg.id
        key = (from_channel_link, message_id)
        
        # Дополнительная защита: проверяем, не обрабатывается ли уже этот пост
        if key in _processing_posts:
            logging.info(f"🔄 [EVENT] Пост {message_id} из {from_channel_link} уже обрабатывается - ПРОПУСКАЕМ")
            return False
        
        # БЛОКИРОВКА: Используем asyncio.Lock для предотвращения race condition
        # Это гарантирует, что только один обработчик событий может проверять/сохранять одновременно
        async with _forward_lock:
            # Проверяем дубликаты с блокировкой
            last_forwarded = read_last_forwarded_events()
            
            if key in last_forwarded:
                logging.info(f"🔄 [EVENT] Пост {message_id} из {from_channel_link} уже переслан - ПРОПУСКАЕМ")
                return False
            
            # Помечаем пост как обрабатываемый
            _processing_posts[key] = True
            
            # ВАЖНО: Сохраняем запись СРАЗУ после проверки, ДО пересылки
            # Это предотвращает повторную пересылку при дублировании событий
            last_forwarded[key] = True
            write_last_forwarded_events(last_forwarded)
            logging.info(f"🔒 [EVENT] Заблокирован пост {message_id} из {from_channel_link} (запись сохранена до пересылки)")
        
        try:
            logging.info(f"[EVENT] New post detected: {message_id} from {from_channel_link}")
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                for channel_type, to_channel_link in to_channels:
                    try:
                        # Преобразуем ссылку в формат для Bot API
                        if '/' in to_channel_link:
                            channel_username = to_channel_link.split('/')[-1]
                            to_username = f"@{channel_username}"
                        elif to_channel_link.startswith('@'):
                            to_username = to_channel_link
                        else:
                            to_username = f"@{to_channel_link}"
                        
                        logging.info(f"[BOT] Sending post {message_id} to aggregator: {to_username}")
                        
                        # Пытаемся использовать forwardMessage
                        ...
                        async with session.post(forward_url, json=forward_payload) as response:
                            result = await response.json()
                            
                            if result.get('ok'):
                                logging.info(f"[BOT] Sent successfully: {message_id} → {to_username}")
                                success_count += 1
                            else:
                                error_desc = result.get('description', 'Unknown')
                                logging.error(f"❌ [BOT] forwardMessage failed: {error_desc}")
                                
                                # Пробуем copyMessage как fallback
                                logging.info(f"[BOT] Trying copyMessage as fallback...")
                                copy_url = f"https://api.telegram.org/bot{bot_token}/copyMessage"
                                
                                # Получаем текст и медиа из сообщения
                                copy_payload = {
                                    'chat_id': to_username,
                                    'from_chat_id': source_chat_id,
                                    'message_id': message_id
                                ...
                                async with session.post(copy_url, json=copy_payload) as copy_response:
                                    copy_result = await copy_response.json()
                                    if copy_result.get('ok'):
                                        logging.info(f"[BOT] Sent successfully via copyMessage: {message_id} → {to_username}")
                                        success_count += 1
                                    else:
                                        logging.error(f"❌ [BOT] copyMessage also failed: {copy_result.get('description', 'Unknown')}")
                    
                    except Exception as e:
                        logging.error(f"❌ [BOT] Ошибка пересылки в {channel_type} канал: {e}")
                        continue
                
                # Запись уже сохранена выше, просто логируем результат
                if success_count > 0:
                    logging.info(f"✅ [BOT] Успешно переслано в {success_count}/{len(to_channels)} каналов")
                    return True
                else:
                    logging.warning(f"⚠️ [BOT] Не удалось переслать пост {message_id} ни в один канал, но запись уже сохранена")
                    return False
        finally:
            # Удаляем пост из списка обрабатываемых
            _processing_posts.pop(key, None)
            
    except Exception as e:
        logging.error(f"❌ [BOT] Критическая ошибка при пересылке через Bot API: {e}")
        # Удаляем пост из списка обрабатываемых даже при ошибке
        _processing_posts.pop(key, None)
        return False

async def setup_forwarding_events(client, forward_config, config):
    """
    Настраивает обработчики событий Telethon для мгновенной пересылки постов
    
    Args:
        client: Telethon клиент
        forward_config: Конфигурация пересылки
        config: Общая конфигурация
    """
    bot_token = config.get('BOT_TOKEN', '')
    if not bot_token:
        logging.error("❌ [EVENTS] BOT_TOKEN не указан, event-based пересылка отключена")
        return
    ...
    # Формируем список каналов назначения
    to_channels = []
    if to_channel:
    ...
    if not to_channels:
        logging.error("❌ [EVENTS] Не указаны каналы назначения")
        return
    
    if not from_channels:
        logging.error("❌ [EVENTS] Не указаны каналы-источники")
        return
    
    # Получаем entity каналов-источников для events
    source_entities = []
    for from_channel in from_channels:
        try:
            entity = await resolve_channel_link(client, from_channel)
            if entity:
                source_entities.append((from_channel, entity))
                logging.info(f"✅ [EVENTS] Канал-источник добавлен: {from_channel} ({getattr(entity, 'title', 'N/A')})")
        except Exception as e:
            logging.error(f"❌ [EVENTS] Ошибка получения entity для {from_channel}: {e}")
    
    if not source_entities:
        logging.error("❌ [EVENTS] Не удалось получить ни одного канала-источника")
        return
    
    logging.info(f"✅ [EVENTS] Настроено {len(source_entities)} обработчиков событий для {len(to_channels)} каналов назначения")
    
    # Создаем обработчик событий для каждого канала-источника
    @client.on(events.NewMessage(chats=[entity for _, entity in source_entities]))
    async def handler(event):
        """Обработчик нового сообщения - мгновенная пересылка"""
        try:
            # Определяем, из какого канала пришло сообщение
            from_channel_link = None
            event_chat_id = None
            
            # Получаем ID канала из события
            if hasattr(event.message.peer_id, 'channel_id'):
                event_chat_id = event.message.peer_id.channel_id
            elif hasattr(event.message, 'chat_id'):
                event_chat_id = event.message.chat_id
            
            # Сопоставляем с нашими каналами-источниками
            for channel_link, entity in source_entities:
                try:
                    entity_id = None
                    if hasattr(entity, 'id'):
                        entity_id = entity.id
                    
                    if event_chat_id and entity_id and event_chat_id == entity_id:
                        from_channel_link = channel_link
                        break
                    
                    # Альтернативная проверка через username
                    if hasattr(entity, 'username') and hasattr(event.message.chat, 'username'):
                        if entity.username == event.message.chat.username:
                            from_channel_link = channel_link
                            break
                except Exception as e:
                    logging.debug(f"Ошибка при определении канала: {e}")
                    continue
            
            if not from_channel_link:
                # Последняя попытка: проверяем по get_entity
                try:
                    chat = await event.get_chat()
                    for channel_link, entity in source_entities:
                        if hasattr(chat, 'id') and hasattr(entity, 'id'):
            ...
            if not from_channel_link:
                logging.warning(f"⚠️ [EVENT] Не удалось определить канал-источник для поста {event.message.id}")
                logging.debug(f"   Event chat_id: {event_chat_id}, Source entities: {[(link, getattr(e, 'id', None)) for link, e in source_entities]}")
                return
            
            logging.info(f"[EVENT] New post detected: {event.message.id} from {from_channel_link}")
            
            # Мгновенная пересылка через Bot API
            success = await forward_post_via_bot_api(
                event.message,
                from_channel_link,
                to_channels,
                bot_token,
            ...
            if success:
                # Запускаем фоновые задачи (реакции, комментарии, избранное) асинхронно
                logging.info("[ASYNC] Background tasks started")
                # TODO: Добавить запуск реакций, комментариев, избранного через asyncio.create_task()
                # Это можно сделать здесь, если нужен этот функционал при event-based пересылке
                
        except Exception as e:
            logging.error(f"❌ [EVENT] Ошибка в обработчике событий: {e}")

# ============================================================================
# СТАРЫЕ ФУНКЦИИ (polling-based, deprecated)
# ============================================================================
    ...
    try:
        ...
        for from_channel in from_channels:
            try:
                ...
                # ============================================================
                # DEPRECATED: POLLING-BASED ПЕРЕСЫЛКА (закомментировано)
                # Используйте event-based пересылку через setup_forwarding_events()
                # ============================================================
                ...
                logging.warning(f"⚠️ [DEPRECATED] Polling-based пересылка отключена. Используйте event-based пересылку.")
                continue  # Пропускаем старую логику
...
async def run_bot_cycle(config):
    ...
    try:
        ...
        # ПЕРВЫЙ ЦИКЛ: Реакции и Избранное (пересылка работает через event-based систему)
        logging.info("=== ПЕРВЫЙ ЦИКЛ: Реакции + Избранное ===")
        
        # ПЕРЕСЫЛКА ОТКЛЮЧЕНА В ОСНОВНОМ ЦИКЛЕ - используется event-based пересылка
        # Event-based пересылка настроена отдельно и работает автоматически при появлении новых постов
        if config.get_bool('ENABLE_FORWARDING', False):
            logging.info("ℹ️ Пересылка постов работает через event-based систему (настроена отдельно, не требует polling)")
...
async def main():
    ...
    # ============================================================================
    # EVENT-BASED FORWARDING: Настройка мгновенной пересылки через Telethon events
    # ============================================================================
    event_forwarding_client = None
    if config.get_bool('ENABLE_FORWARDING', False):
        logging.info("🔄 Настройка EVENT-BASED пересылки постов...")
        forward_config = config.get_forward_config()
        
        # Загружаем аккаунты для event-based пересылки
        all_accounts = load_accounts()
        if all_accounts:
            # Используем первый аккаунт для событий
            event_account = all_accounts[0]
            try:
                if await connect_account(event_account):
                    event_forwarding_client = event_account.client
                    logging.info(f"✅ Аккаунт {event_account.phone} подключен для event-based пересылки")
                    
                    # Настраиваем обработчики событий
                    await setup_forwarding_events(event_forwarding_client, forward_config, config)
                    logging.info("✅ Event-based пересылка настроена и работает")
                else:
                    logging.error("❌ Не удалось подключить аккаунт для event-based пересылки")
            except Exception as e:
                logging.error(f"❌ Ошибка настройки event-based пересылки: {e}")
        else:
            logging.error("❌ Нет аккаунтов для event-based пересылки")
    
    # ============================================================================
    # DEPRECATED: Старый polling-based цикл пересылки (закомментирован)
    # ============================================================================
    # Старый код с FORWARDING_INTERVAL и polling закомментирован
    # Теперь используется event-based пересылка выше
    ...
    try:
        ...
        # Отключаем клиент для event-based пересылки
        if event_forwarding_client and event_forwarding_client.is_connected():
            try:
                await event_forwarding_client.disconnect()
                logging.info("✅ Event-based клиент отключен")
            except Exception as e:
                logging.warning(f"⚠️ Ошибка отключения event-based клиента: {e}")
